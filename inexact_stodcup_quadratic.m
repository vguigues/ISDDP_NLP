...
function [lower_bounds_sddp,upper_bounds_sddp,time_sddp,nb_f_unsolved,nb_b_unsolved]=inexact_stodcup_quadratic(T,n,M,iter_stodcup,nb_iter_max,xis,Us,Psis,tol,x0,Minit,probabilities,accuracies,alpha)
...
for t=1:T
    ...
    for j=1:Maux
        ...
        for i=1:Minit
            ...
            v=x1-x2;
            f1=v'*(xi*xi'+alpha*eye(n))*v+x1'*xi+1;
            f2=x1'*(xi*xi'+alpha*eye(n))*x1+x1'*ones(n,1)+ur;
            [vf,indf]=max([f1,f2]);
            g1=4*((x1-ones(n,1))'*(x1-ones(n,1)));
            g2=x1'*(xi*xi'+alpha*eye(n))*x1+x1'*xi+1;
            [vg,indg]=max([g1,g2]);
            ...
            if (indf==1)
                if (t==1)
                    ...
                    valijstodcup{1,t}{1,j}=[valijstodcup{1,t}{1,j},1,-2*(x1-x2)'*(xi*xi'+alpha*eye(n))-xi'];
                    bs{1,t}{1,j}=[bs{1,t}{1,j};zeros(1,n)];
                    cs{1,t}{1,j}=[cs{1,t}{1,j};1-(x1-x2)'*(xi*xi'+alpha*eye(n))*(x1+x2)];
                else
                    subistodcup{1,t}{1,j}=[subistodcup{1,t}{1,j},counters(t,j)*ones(1,n+1)];
                    subjstodcup{1,t}{1,j}=[subjstodcup{1,t}{1,j},1,[3:1:n+2]];
                    valijstodcup{1,t}{1,j}=[valijstodcup{1,t}{1,j},1,-2*(x1-x2)'*(xi*xi'+alpha*eye(n))-xi'];
                    bs{1,t}{1,j}=[bs{1,t}{1,j};2*(x2-x1)'*(xi*xi'+alpha*eye(n))];
                    cs{1,t}{1,j}=[cs{1,t}{1,j};1-(x1-x2)'*(xi*xi'+alpha*eye(n))*(x1-x2)];
                end
            else
                subistodcup{1,t}{1,j}=[subistodcup{1,t}{1,j},counters(t,j)*ones(1,n+1)];
                subjstodcup{1,t}{1,j}=[subjstodcup{1,t}{1,j},1,[3:1:n+2]];
                valijstodcup{1,t}{1,j}=[valijstodcup{1,t}{1,j},1,-2*x1'*(xi*xi'+alpha*eye(n))-ones(1,n)];
                cs{1,t}{1,j}=[cs{1,t}{1,j};ur-x1'*(xi*xi'+alpha*eye(n))*x1];
                bs{1,t}{1,j}=[bs{1,t}{1,j};zeros(1,n)];
            end
            if (indg==1)
                ...
                subistodcupg{1,t}{1,j}=[subistodcupg{1,t}{1,j},counters(t,j)*ones(1,n)];
                subjstodcupg{1,t}{1,j}=[subjstodcupg{1,t}{1,j},[3:1:n+2]];
                valijstodcupg{1,t}{1,j}=[valijstodcupg{1,t}{1,j},xi'+2*x1'*(xi*xi'+alpha*eye(n))];
                hs{1,t}{1,j}=[hs{1,t}{1,j};psir+x1'*(xi*xi'+alpha*eye(n))*x1-1];
            end
        end
    end
end
...
while End_Algo
    ...
    if (iter<=iter_stodcup)
        ...
        if (indf==1)
            ...
            valijstodcup{1,1}{1,1}=[valijstodcup{1,1}{1,1},1,-2*(x1-x2)'*(xi*xi'+alpha*eye(n))-xi'];
            cs{1,1}{1,1}=[cs{1,1}{1,1};1-(x1-x2)'*(xi*xi'+alpha*eye(n))*(x1+x2)];
        else
            subistodcup{1,1}{1,1}=[subistodcup{1,1}{1,1},counters(1,1)*ones(1,n+1)];
            subjstodcup{1,1}{1,1}=[subjstodcup{1,1}{1,1},1,[3:1:n+2]];
            valijstodcup{1,1}{1,1}=[valijstodcup{1,1}{1,1},1,-2*x1'*(xi*xi'+alpha*eye(n))-ones(1,n)];
            cs{1,1}{1,1}=[cs{1,1}{1,1};ur-x1'*(xi*xi'+alpha*eye(n))*x1];
        end
        if (indg==1)
            ...
            subistodcupg{1,1}{1,1}=[subistodcupg{1,1}{1,1},counters(1,1)*ones(1,n)];
            subjstodcupg{1,1}{1,1}=[subjstodcupg{1,1}{1,1},[3:1:n+2]];
            valijstodcupg{1,1}{1,1}=[valijstodcupg{1,1}{1,1},xi'+2*x1'*(xi*xi'+alpha*eye(n))];
            hs{1,1}{1,1}=[hs{1,1}{1,1};psir+x1'*(xi*xi'+alpha*eye(n))*x1-1];
        end
        ...
        for t=2:T-1
            ...
            if (indf==1)
                ...
                valijstodcup{1,t}{1,Index}=[valijstodcup{1,t}{1,Index},1,-2*(x1-x2)'*(xi*xi'+alpha*eye(n))-xi'];
                bs{1,t}{1,Index}=[bs{1,t}{1,Index};2*(x2-x1)'*(xi*xi'+alpha*eye(n))];
                cs{1,t}{1,Index}=[cs{1,t}{1,Index};1-(x1-x2)'*(xi*xi'+alpha*eye(n))*(x1-x2)];
            else
                subistodcup{1,t}{1,Index}=[subistodcup{1,t}{1,Index},counters(t,Index)*ones(1,n+1)];
                subjstodcup{1,t}{1,Index}=[subjstodcup{1,t}{1,Index},1,[3:1:n+2]];
                valijstodcup{1,t}{1,Index}=[valijstodcup{1,t}{1,Index},1,-2*x1'*(xi*xi'+alpha*eye(n))-ones(1,n)];
                cs{1,t}{1,Index}=[cs{1,t}{1,Index};ur-x1'*(xi*xi'+alpha*eye(n))*x1];
            ...
            if (indg==1)
                ...
                subistodcupg{1,t}{1,Index}=[subistodcupg{1,t}{1,Index},counters(t,Index)*ones(1,n)];
                subjstodcupg{1,t}{1,Index}=[subjstodcupg{1,t}{1,Index},[3:1:n+2]];
                valijstodcupg{1,t}{1,Index}=[valijstodcupg{1,t}{1,Index},xi'+2*x1'*(xi*xi'+alpha*eye(n))];
                hs{1,t}{1,Index}=[hs{1,t}{1,Index};psir+x1'*(xi*xi'+alpha*eye(n))*x1-1];
            end
        end
        ...
        while ((t<=T)&&(contloop))    
            ...
            if (t==1)
                prob.buc=[-1-x0'*(xi*xi'+alpha*eye(n))*x0;-ur;-4*n;-1;psir;inf*ones(iter,1)];
            else
                if (t<T)
                    prob.buc=[-1-trial_states{1,t-1}'*(xi*xi'+alpha*eye(n))*trial_states{1,t-1};-ur;-4*n;-1;psir;inf*ones(iter,1)];
                else
                    prob.buc=[-1-trial_states{1,t-1}'*(xi*xi'+alpha*eye(n))*trial_states{1,t-1};-ur;-4*n;-1;psir];
                end     
            end
            if (t<T)
            ...
            if isempty(strfind(res.rcodestr, 'MSK_RES_ERR'))
                ...
                solsta=strcat('MSK_SOL_STA_', res.sol.itr.solsta);
                if (strcmp(solsta,'MSK_SOL_STA_PRIMAL_INFEASIBLE_CER')==1)
                    disp('Unfeasible primal problem - Forward pass');
                    contloop=1;
                ...
                elseif (strcmp(solsta,'MSK_SOL_STA_DUAL_INFEASIBLE_CER')==1)
                    disp('Primal infinite optimal value - Forward pass');
                    contloop=1;
                    nb_f_unsolved=nb_f_unsolved+1;
                    if (t<T)
                        trial_states{1,t}=-100*ones(n,1)+200*rand(n,1);;
                ...
                elseif (strcmp(solsta,'MSK_PRO_STA_UNKNOWN')==1)
                    disp('Unknown status - Forward pass')
                    contloop=1;
                    nb_f_unsolved=nb_f_unsolved+1;
                    if (t<T)
                        trial_states{1,t}=-100*ones(n,1)+200*rand(n,1);;
    ...
    iter=iter+1;
        ...
        while (t>=2)
            ...
            while ((j<=M)&&(contloop))
                ...
                if (t==T)
                    prob.blx=[-inf;0;-100*ones(n,1);-inf;-100*ones(n,1)];
                    prob.bux=[inf;0;100*ones(n,1);inf;100*ones(n,1)];
                else
                    prob.blx=[-inf;-10^(10);-100*ones(n,1);-inf;-100*ones(n,1)];
                    prob.bux=[inf;inf;100*ones(n,1);inf;100*ones(n,1)];
                end
                if (t==T)
               ...
               if isempty(strfind(res.rcodestr, 'MSK_RES_ERR'))
                   ...
                   solsta=strcat('MSK_SOL_STA_', res.sol.itr.solsta);
                   if (strcmp(solsta,'MSK_SOL_STA_PRIMAL_INFEASIBLE_CER')==1)
                       disp('Unfeasible primal problem - Backward');
                       contloop=0;
                       nb_b_unsolved=nb_b_unsolved+1;
                   elseif (strcmp(solsta,'MSK_SOL_STA_DUAL_INFEASIBLE_CER')==1)
                       disp('Primal infinite optimal value - Backward');
                       contloop=0;
                       nb_b_unsolved=nb_b_unsolved+1;
                   elseif (strcmp(solsta,'MSK_PRO_STA_UNKNOWN')==1)
                       disp('Unknown status - Backward');
                       contloop=0;
                       nb_b_unsolved=nb_b_unsolved+1;
                   else
                       sol=res.sol.itr.xx;
                       if (t==T)
                           dual=res.sol.itr.slc(5+1:5+n)-res.sol.itr.suc(5+1:5+n);
                       else
                           dual=res.sol.itr.slc(5+iter+2:5+iter+1+n)-res.sol.itr.suc(5+iter+2:5+iter+1+n);
                       end
                       slope=slope+dual;
                       intercept=intercept+sol(1)+sol(2)-abs(res.sol.itr.pobjval-res.sol.itr.dobjval)-dual'*trial_states{1,t-1};
                   end
               else
                   contloop=0;
                   nb_b_unsolved=nb_b_unsolved+1;
